...
static int outer_child(
        ...
        if (!arg_mstack && arg_volatile_mode != VOLATILE_NO) {
                ...
                if (root_fd < 0 && errno == EINVAL) {
                        log_debug("Recursive clone of '%s' refused (locked or unbindable submounts?), "
                                  "falling back to non-recursive clone; nested submounts will not "
                                  "be preserved.", directory);
                        root_fd = open_tree(
                                        AT_FDCWD, directory,
