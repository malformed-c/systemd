...
static int outer_child(
        ...
        if (!IN_SET(arg_userns_mode, USER_NAMESPACE_NO, USER_NAMESPACE_MANAGED) &&
            ...
            chown_uid != 0) {
                ...
                /* --volatile=yes only ever reaches here via the synthetic --directory=/--image= MStack
                 * wrapping above (--mstack= requires managed userns or userns off, so it never reaches
                 * this PICK/FIXED-only block) - and that path already idmapped its extracted /usr/ itself,
                 * in mstack_make_mounts(), while it was still a fresh, unattached clone (the same
                 * "individual layers before merging" restriction mstack's own overlay construction already
                 * has to work around applies here too: idmapping this path now, after mstack merged and
                 * attached it, would hit the kernel's already-merged-overlay restriction and fail). */
                if ((dissected_image && dissected_image->partitions[PARTITION_USR].found) ||
                    (arg_volatile_mode == VOLATILE_YES && chown_uid == 0)) {
