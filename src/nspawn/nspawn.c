 ...
 * in one place because the pieces interact: --volatile= is always forced together with arg_read_only (see
 * the argument post-processing further up), but that must never make the volatile mount itself read-only -
 * it is mstack's own synthetic writable layer that has to stay writable. Only bind@/robind@ entries track
 * an explicitly requested --read-only, and only where it was asked for on the command line or via
 * ReadOnly=yes, rather than merely implied by --volatile= or by a custom root mount. */
...
static int outer_child(
        ...
        if (arg_mstack) {
                ...
                /* Which is why tmpfs_uid_shift can stay at the UID_INVALID MSTACK_INIT gave it. */
