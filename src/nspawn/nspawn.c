...
static int determine_remount_idmapping(const char *directory, RemountIdmapping *ret) {
        int r;
...
static int mstack_apply_bind_mounts_late(MStack *mstack, const char *directory, MStackFlags flags) {
        ...
        /* The caller's own bind@/robind@/tmpfs@ entries are applied here, after mount_all(), rather than
         * during the initial root assembly above - mount_all() mounts fresh API VFS filesystems (/proc,
         * /dev, /run, /tmp, /sys), which would otherwise shadow any of these at the same paths. That holds
         * regardless of whether --volatile= is in play: mstack is authoritative for the whole tree either
         * way, so a synthetic --volatile=overlay rw layer (see mstack_merge_volatile()) is already part of
         * the root assembled above, and these land on top of it here.
         *
         * The entries mstack_merge_volatile() synthesizes are the exception: they are marked from_volatile
         * and went up during assembly instead, since they only ever target paths the root itself owns
         * (/var/) and so have nothing here that could shadow them.
         *
        ...
        r = mstack_apply_bind_mounts(mstack, root_fd, directory, flags, /* volatile_only= */ false);
        if (r < 0)
                return r;
...
static int outer_child(
        ...
        if (mount_fd >= 0) {
                ...
                /* Decouple arg_read_only from mstack binds: make bind@ mounts read-only only where
                 * read-only was actually asked for - on the command line, or via ReadOnly=yes in a .nspawn
                 * file, which sets arg_read_only without touching arg_settings_mask - and not where it was
                 * merely implied further up, by --volatile= or by a custom root mount. */
                ...
                /* Defer the caller's own bind mounts to after mount_all(), so that API VFS mounts
                 * (/proc, /sys, /dev, /run, /tmp) don't shadow them. The root, /usr, and whatever
                 * --volatile= synthesizes are all attached immediately instead. */
        ...
        /* chown_uid cannot be known any earlier than this: determine_uid_shift() above is what settles
         * arg_uid_shift, and that in turn has to run against an already-attached root - so the --mstack=
         * case necessarily merged --volatile= into 'mstack' without it. Patch in the uid=/gid= tmpfs parity
         * now, for the tmpfs entries that have yet to be realized: the caller's own bind@/robind@/tmpfs@,
         * which MSTACK_DEFER_MOUNT holds back until the mstack_apply_bind_mounts() pass after mount_all().
         * Matches upstream's existing "0 means no shift needed" convention for uid_shift.
         *
         * Note this does NOT reach the entries mstack_merge_volatile() synthesizes: those are marked
         * from_volatile and attached during assembly, above, so a --volatile=state tmpfs@/var has already
         * been realized by the time we get here. That is only correct because --mstack= is restricted to
         * managed userns or userns off (enforced with a clean error in run(), see the EOPNOTSUPP for
         * "--mstack= requires managed user namespacing"), which means in_child_chown() is false and
         * chown_uid is always 0 on this path - i.e. the value patched in below is UID_INVALID either way,
         * and realizing those tmpfs mounts early cannot have picked up the wrong owner. Assert it, so that
         * lifting the userns restriction fails loudly here instead of silently producing a host-root-owned
        ...
        if (arg_mstack) {
                ...
                /* --volatile=overlay's synthetic writable rw layer was likewise already fully realized by
                 * mstack_make_mounts() further up, before chown_uid
